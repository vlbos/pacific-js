...
export class OpenSeaAPI {
    ...
    public async postOrder(order: OrderJSON, retries = 2): Promise<Order> {
        ...
        try {
            ...
            submit(this.apip, this.apip.tx.orderbook.postOrder(order_id, users().bob.key.address, order), users().betty)
    ...
    public async postAssetWhitelist(
        ...
        //   email
        // })
        const json = submit(this.apip, this.apip.tx.orderbook.postAssetWhiteList(tokenAddress, tokenId, email), users().betty)
    ...
    public async getOrder(query: OrderQuery): Promise<Order> {
        ...
        const result = await this.apip.rpc.orderbook.getOrder(query)
    ...
    public async getOrders(
        ...
        const result = await this.apip.rpc.orderbook.getOrders(query, page)
    ...
    public async getAsset({
        ...
        try {
            ...
            json = await this.apip.rpc.orderbook.getAsset(stringToHex(tokenAddress), stringToHex(tokenId + ""))
    ...
    public async getAssets(
        ...
        // }
        const json = await this.apip.rpc.orderbook.getAssets(query, 1)
        return {
